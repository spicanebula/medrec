...
export default class medication_modify extends Component<{}> {
  constructor(){
    super();
    this.state = { 
      medication: null, 
  ...
  _handleStatus = (itemValue) => {
    var m = this.state.medication;
    m.status = itemValue;
    this.setState({ medication: m });
  };

  _handleFormSave = () => { alert(this.props.medication); };
    ...
    if(1==1){
      return(
        <View >
          ...
          <TextInput value = {this.state.medication.provider_fname} />
    ...
    if(1==1){
      return(
        <View >
          ...
          <TextInput value={this.state.medication.name}/>
        </View>
      );
    }else{
      return null;
    ...
    if(1==1){
      return(
        <View >
          ...
          <TextInput value={this.state.medication.directions}/>
        </View>
      );
    }else{
      return null;
    ...
    if(1==1){
      return(
        <View >
          ...
          <TextInput multiline = {true} numberOfLines = {4} value = {this.state.medication.comments} />
        </View>
      );
    }else{
      return null;
